...
// Massively trimmed to be just an INTERCEPT type AI, with increasing tracking that continues to home in on the targets last position if it dies. 
...
public class ASF_celpekProjScript extends BaseEveryFrameCombatPlugin {

	//The projectiles "base" turn rate, in degrees/second (starts at 50% strength, scales up over time)
	private static final float TURN_RATE = 35f;
	...
	private static final float SWAY_AMOUNT_PRIMARY = 5.2f;
	...
	//Secondary and primary sway both run in parallel, allowing double-sine swaying if desired
	private static final float SWAY_PERIOD_PRIMARY = 0.875f;
	private static final float SWAY_PERIOD_SECONDARY = 0.45f;
	...
	private static final int INTERCEPT_ITERATIONS = 7;
	...
	private float turnMult; // Mow much the turn rate has increased over time
	 ...
	 */
	public ASF_celpekProjScript(@NotNull DamagingProjectileAPI proj, CombatEntityAPI target) {
		this.proj = proj;
		this.target = target;
		lastTargetPos = MathUtils.getPointOnCircumference(proj.getLocation(), 6000, proj.getFacing());
		swayCounter1 = MathUtils.getRandomNumberInRange(0f, 1f);
		swayCounter2 = MathUtils.getRandomNumberInRange(0f, 1f);
		lifeCounter = 0f;
		turnMult = 0.5f; // we start with 50% of base turn rate
	...
	public void advance(float amount, List<InputEventAPI> events) {
		...
		turnMult += amount;
		float moddedTurn = TURN_RATE * turnMult;
		if (lifeCounter > 0.11f && lifeCounter < 1.1f) {
			moddedTurn += 90f; // increase turn rate over time, with a spike between 0.11s and 1.1s
		}
		...
		//Start our guidance stuff...
			...
			facingSwayless += Misc.getClosestTurnDirection(facingSwayless, angleToHit) * Math.min(angleDiffAbsolute, moddedTurn*amount);
