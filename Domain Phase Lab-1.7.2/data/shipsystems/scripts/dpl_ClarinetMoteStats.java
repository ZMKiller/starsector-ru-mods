...
public class dpl_ClarinetMoteStats extends BaseShipSystemScript{
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		dpl_ClarinetMoteData data = (dpl_ClarinetMoteData) Global.getCombatEngine().getCustomData().get(key);
		...
		if (state == State.IN) {
			if (time >= fireThreshold) {
				for (WeaponAPI w : ship.getAllWeapons()) {
					if (w.isDecorative() && w.getSpec().hasTag("dpl_clarinet_system") && data.count > 0) {
						w.setGlowAmount(effectLevel, w.getSpec().getGlowColor());
						w.setForceFireOneFrame(true);
						data.count -= 1;
						ship.getFluxTracker().increaseFlux(350f, false);
					}
				}
				fireThreshold += 0.5f;
			}
			...
			if (time >= fireThreshold) {
				...
				for (WeaponAPI w : ship.getAllWeapons()) {
					if (w.isDecorative() && w.getSpec().hasTag("dpl_clarinet_system")) {
	...
	public String getInfoText(ShipSystemAPI system, ShipAPI ship) {
		...
		if (target != null) {
			int motes = data.count;
			if (motes == 0) {
				return "NO ANOMALY AVAILABLE";
			} else if (ship.isPhased()){
				return "SHIP CANNOT BE PHASED";
			} else {
				return "READY";
			}
		}
	...
	public boolean isUsable(ShipSystemAPI system, ShipAPI ship) {
		...
		boolean hasClarinetMote = ship.getVariant().hasHullMod("dpl_clarinet_mote");
		return (hasTarget && hasClarinetMote && !isPhased);
	}
}
