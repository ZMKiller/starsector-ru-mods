 ...
 */
public class dpl_phaselightstationEffect implements OnHitEffectPlugin, OnFireEffectPlugin {
		...
		if (target instanceof ShipAPI) {
			...
			if (!ship.isFighter() && !ship.isDrone()) {
				...
				if (!shieldHit) {
					...
					float realdam = projectile.getDamageAmount();
					ship.setHitpoints(Math.max(ship.getHitpoints() - realdam, 1));
					engine.addFloatingDamageText(point, realdam, Misc.FLOATY_HULL_DAMAGE_COLOR, ship, projectile.getSource());
					if (ship.getHitpoints() <= 2 && !ship.getVariant().hasHullMod("vastbulk")) {
						//Just kill it if its hull point is too low.
						engine.applyDamage(ship, ship.getLocation(), 10*ship.getMaxHitpoints(), DamageType.HIGH_EXPLOSIVE, 0f, true, false, null);
					}
				} else if (piercedShield) {
					float emp = projectile.getEmpAmount();
					float dam = projectile.getDamageAmount()*0.25f; // this should be 1 for regular and a bunch for high-frequency
					...
					float realdam = projectile.getDamageAmount();
					ship.getFluxTracker().increaseFlux(realdam, true);
				} else {
					float realdam = projectile.getDamageAmount();
					ship.getFluxTracker().increaseFlux(realdam, true);
				}
