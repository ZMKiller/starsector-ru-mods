 ...
 */
public class dpl_doom_flamerEffect extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
																					OnHitEffectPlugin,
																					EveryFrameWeaponEffectPlugin {

	public dpl_doom_flamerEffect() {
	}
	
	protected List<dpl_doom_flamerEffect> trails;
	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		if (trails == null) return;
		
		Iterator<dpl_doom_flamerEffect> iter = trails.iterator();
		while (iter.hasNext()) {
		...
		if (weapon.getShip() != null) {
			...
			for (dpl_doom_flamerEffect curr : trails) {
				if (curr.proj != null) {
		...
		for (int i = 0; i < numIter; i++) { 
			for (dpl_doom_flamerEffect trail : trails) {
...
//								   rampUp, dur, inverted);
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		...
		dpl_doom_flamerEffect trail = new dpl_doom_flamerEffect(projectile, prev);
		...
		if (trails == null) {
			trails = new ArrayList<dpl_doom_flamerEffect>();
		}
		trails.add(0, trail);
	}
...
//			offset.y += vel.y * 1f;
	...
	public dpl_doom_flamerEffect(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
		this.proj = proj;
		this.prev = prev;
