 ...
 */
public class dpl_quaverEffect implements OnHitEffectPlugin, OnFireEffectPlugin {
		...
		if (target instanceof ShipAPI) {
			...
			if (!ship.isFighter() && !ship.isDrone()) {
				...
				if (!shieldHit) {
					float emp = projectile.getEmpAmount();
					float dam = projectile.getDamageAmount()*0.25f; // this should be 1 for regular and a bunch for high-frequency
					float Hulldam = projectile.getDamageAmount();
					engine.applyDamage(projectile, target, point, 
							Hulldam, DamageType.ENERGY, 0f, false, false, projectile.getSource(), true);
