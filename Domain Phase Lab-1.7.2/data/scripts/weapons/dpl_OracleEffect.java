...
public class dpl_OracleEffect implements BeamEffectPlugin {
	...
	public void advance(float amount, CombatEngineAPI engine, BeamAPI beam) {
		...
		if (target instanceof ShipAPI && beam.getBrightness() >= 0.9f) {
            ...
            if (!hitShield){
            float dam2 = beam.getDamage().getDamage() + beam.getDamageTarget().getHitpoints()*1.25f;
            engine.applyDamage(beam.getDamageTarget(), beam.getRayEndPrevFrame(), dam2, DamageType.HIGH_EXPLOSIVE, 0f, true, false, ship, false);
            }
	...
	public void allStationTargets(CombatEntityAPI ship, CombatEngineAPI engine) {
		...
		while (iter.hasNext()) {
			...
			float dam3 = ship.getHitpoints()*1.5f;
			engine.spawnEmpArcPierceShields(
