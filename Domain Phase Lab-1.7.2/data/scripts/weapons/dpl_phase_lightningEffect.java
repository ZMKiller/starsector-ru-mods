		...
		if (target != null) {
			...
			if (target instanceof MissileAPI) {
				...
				if (enemy.isFighter() || enemy.isDrone() || enemy.isShuttlePod()) {
					Global.getCombatEngine().applyDamage(projectile, target, arc.getTargetLocation(), 
							enemy.getMaxHitpoints()*0.5f, DamageType.HIGH_EXPLOSIVE, 0f, true, false, projectile.getSource(), true);
				}
			}
			...
			if (new_target != null) {
				...
				if (new_target instanceof MissileAPI) {
					Global.getCombatEngine().applyDamage(projectile, new_target, arc.getTargetLocation(), 
							dam*3f, DamageType.HIGH_EXPLOSIVE, 0f, true, false, projectile.getSource(), true);
					...
					ShipAPI enemy = (ShipAPI) new_target;
					if (enemy.isFighter() || enemy.isDrone() || enemy.isShuttlePod()) {
						Global.getCombatEngine().applyDamage(projectile, new_target, arc.getTargetLocation(), 
								enemy.getMaxHitpoints()*0.5f, DamageType.HIGH_EXPLOSIVE, 0f, true, false, projectile.getSource(), true);
					}
				...
				if (new_2target != null) {
					...
					if (new_2target instanceof MissileAPI) {
						Global.getCombatEngine().applyDamage(projectile, new_2target, arc.getTargetLocation(), 
								dam*3f, DamageType.HIGH_EXPLOSIVE, 0f, true, false, projectile.getSource(), true);
						...
						ShipAPI enemy = (ShipAPI) new_2target;
						if (enemy.isFighter() || enemy.isDrone() || enemy.isShuttlePod()) {
							Global.getCombatEngine().applyDamage(projectile, new_2target, arc.getTargetLocation(), 
									enemy.getMaxHitpoints()*0.5f, DamageType.HIGH_EXPLOSIVE, 0f, true, false, projectile.getSource(), true);
						}
