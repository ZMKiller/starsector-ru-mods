...
public class FoodForge extends BaseToggleAbility {
	public static final Color CONTRAIL_COLOR = new Color(255, 97, 27, 80);

    public float getFAMINEWPerFOOD() {
        return (Global.getSettings().getFloat("MF_FOODConversionRate")
                * (Global.getSector().getEconomy().getCommoditySpec(Commodities.HEAVY_MACHINERY).getBasePrice() + 5*Global.getSector().getEconomy().getCommoditySpec(Commodities.FOOD).getBasePrice())
                / Global.getSector().getEconomy().getCommoditySpec(jydr_Items.FAMINEW).getBasePrice())
                ;//* ;
    }
    public int MFOODCoom() {
        return Global.getSettings().getInt("MFUseExtraCommodities");
    }
    float FOODCost = Global.getSettings().getFloat("MF_FOODCost");
    float HeavyMachineryCost = Global.getSettings().getFloat("MF_HeavyMachineryCost");
    float CorruptedFOODMultiplier = Global.getSettings().getFloat("MF_CorruptedFOOD");
    float PristineFOODMultiplier = Global.getSettings().getFloat("MF_PristineFOOD");
    ...
    protected String getActivationText() {
        ...
        && Commodities.FOOD != null
        && getFleet() != null
        || (getFleet().getCargo().getCommodityQuantity(Commodities.FOOD) <= 0
        || getFleet().getCargo().getCommodityQuantity(Commodities.HEAVY_MACHINERY) <= 0
        || getFleet().getCargo().getFAMINEW() >= getFleet().getCargo().getMaxCapacity())) {
        return null;
        } else */return null;
    }
    ...
    protected void applyEffect(float amount, float level) {
        ...
        fleet.getStats().getDetectedRangeMod().modifyPercent(getModId(), JYDRPlugin.SENSOR_PROFILE_INCREASE_PERCENT, "FAMINEW Forging");

        float days = Global.getSector().getClock().convertToDays(amount);
        float cost = days;
        float supply = fleet.getCargo().getCommodityQuantity(jydr_Items.FAMINEW);
        if (MFOODCoom() > 0) {
            if(fleet.getCargo().getCommodityQuantity(Commodities.FOOD) <= 0 || fleet.getCargo().getCommodityQuantity(Commodities.HEAVY_MACHINERY) <= 0) {
                ...
                fleet.addFloatingText("Full of FAMINEW", Misc.setAlpha(entity.getIndicatorColor(), 255), 0.5f);
                deactivate();
            } else {
                ...
                fleet.getCargo().removeCommodity(Commodities.FOOD, cost*FOODCost*basedmodifier);
                fleet.getCargo().removeCommodity(Commodities.HEAVY_MACHINERY, cost*HeavyMachineryCost*basedmodifier);
                    ...
                    fleet.getCargo().removeCommodity(Commodities.FOOD, cost*FOODCost);
                    fleet.getCargo().removeCommodity(Commodities.HEAVY_MACHINERY, cost*HeavyMachineryCost);
                }
                if (affectOutput) {fleet.getCargo().addCommodity(jydr_Items.FAMINEW, cost*getFAMINEWPerFOOD()*basedmodifier);} else {fleet.getCargo().addCommodity(jydr_Items.FAMINEW, cost*getFAMINEWPerFOOD());}
            ...
            if(fleet.getCargo().getCommodityQuantity(Commodities.FOOD) <= 0 || fleet.getCargo().getCommodityQuantity(Commodities.HEAVY_MACHINERY) <= 0) {
                fleet.addFloatingText("Out of FOOD or Heavy Machinery", Misc.setAlpha(entity.getIndicatorColor(), 255), 0.5f);
                deactivate(); 
            } else if(supply >= fleet.getCargo().getMaxCapacity()) {
                fleet.addFloatingText("Full of FAMINEW", Misc.setAlpha(entity.getIndicatorColor(), 255), 0.5f);
                deactivate();
                ...
                float basedmodifier = iCalculateBonus();
                if (affectInput) {fleet.getCargo().removeCommodity(Commodities.FOOD, cost*FOODCost*basedmodifier);fleet.getCargo().removeCommodity(Commodities.HEAVY_MACHINERY, cost*HeavyMachineryCost*basedmodifier);} else {fleet.getCargo().removeCommodity(Commodities.FOOD, cost*FOODCost);fleet.getCargo().removeCommodity(Commodities.HEAVY_MACHINERY, cost*HeavyMachineryCost);}
                if (affectOutput) {fleet.getCargo().addCommodity(jydr_Items.FAMINEW, cost*getFAMINEWPerFOOD()*basedmodifier);} else {fleet.getCargo().addCommodity(jydr_Items.FAMINEW, cost*getFAMINEWPerFOOD());}
                for (FleetMemberViewAPI view : getFleet().getViews()) {
                    view.getContrailColor().shift("timidhavenoidea", CONTRAIL_COLOR, getActivationDays(), 2, 1f);
                    view.getContrailWidthMult().shift("timidhavenoidea", 6, getActivationDays(), 2, 1f);
    ...
    public float iCalculateBonus() {
        ...
        float iMaxBonus = PristineFOODMultiplier*iShipSize+SalvageModifier*iSalvageCoomer;
        if (iCorrupted > iShipSize) {
            iCorrupted = iShipSize;
        };
        float iBonus = CorruptedFOODMultiplier*iCorrupted+PristineFOODMultiplier*iPristine+SalvageModifier*iSalvageCoomer;
        if (iBonus > iMaxBonus) {
            iBonus = iMaxBonus;
        };
        return iBonus+1;
