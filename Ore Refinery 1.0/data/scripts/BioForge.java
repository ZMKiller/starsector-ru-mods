...
public class BioForge extends BaseToggleAbility {
	public static final Color CONTRAIL_COLOR = new Color(255, 97, 27, 80);
	

    public float getBIOPHOTOVOLTAICSPerORGANICS() {
        return (Global.getSettings().getFloat("MF_ORGANICSConversionRate")
                * (Global.getSector().getEconomy().getCommoditySpec(Commodities.HEAVY_MACHINERY).getBasePrice() + 5*Global.getSector().getEconomy().getCommoditySpec(Commodities.ORGANICS).getBasePrice())
                / Global.getSector().getEconomy().getCommoditySpec(jydr_Items.BIOPHOTOVOLTAICS).getBasePrice())
                ;//* ;				
    }
    public int MORGANICSCoom() {
        return Global.getSettings().getInt("MFUseExtraCommodities");
    }
    float ORGANICSCost = Global.getSettings().getFloat("MF_ORGANICSCost");
    float HeavyMachineryCost = Global.getSettings().getFloat("MF_HeavyMachineryCost");
    float CorruptedORGANICSMultiplier = Global.getSettings().getFloat("MF_CorruptedORGANICS");
    float PristineORGANICSMultiplier = Global.getSettings().getFloat("MF_PristineORGANICS");
    ...
    protected String getActivationText() {
        ...
        && Commodities.ORGANICS != null
        && getFleet() != null
        || (getFleet().getCargo().getCommodityQuantity(Commodities.ORGANICS) <= 0
    ...
    public void createTooltip(TooltipMakerAPI tooltip, boolean expanded) {
        ...
        float pad = 10f;
        tooltip.addPara("Refine Organics using Heavy Machinery forging it into Biophotovoltaic cells.", pad);
            String Supply = Misc.getRoundedValueMaxOneAfterDecimal(getBIOPHOTOVOLTAICSPerORGANICS());
            float iCoom = iCalculateBonus();
            if (iCoom > 1) {Supply = Misc.getRoundedValueMaxOneAfterDecimal(getBIOPHOTOVOLTAICSPerORGANICS()*iCoom);}
            String canOrIs = isActive() ? "are smelting" : "can smelt";
            String Based = iCoom > 1 ? "Nanoforges in your inventory and ships with Salvage Gantry are improving the process of forging Biophotovoltaics by": "You do not possess a nanoforge or a ship with Salvage Gantry that can hasten the process.";
            String Based2 = iCoom > 1 ? Misc.getRoundedValue((iCoom-1)*100) + "%." : "";
            tooltip.addPara("Your fleet's autoforges " + canOrIs + " %s units of Organics with %s units of Heavy Machinery to create %s Biophotovoltaics on a daily basis.",
                        pad, Misc.getTextColor(), Misc.getRoundedValueMaxOneAfterDecimal(ORGANICSCost*iCoom), Misc.getRoundedValueMaxOneAfterDecimal(HeavyMachineryCost*iCoom), Supply);
            if (MORGANICSCoom() > 0) {
                for (int i = 0; i < MORGANICSCoom(); i++) {
    ...
    protected void applyEffect(float amount, float level) {
        ...
        fleet.getStats().getDetectedRangeMod().modifyPercent(getModId(), JYDRPlugin.SENSOR_PROFILE_INCREASE_PERCENT, "BIOPHOTOVOLTAICS Forging");

        float days = Global.getSector().getClock().convertToDays(amount);
        float cost = days;
        float supply = fleet.getCargo().getCommodityQuantity(jydr_Items.BIOPHOTOVOLTAICS);
        if (MORGANICSCoom() > 0) {
            if(fleet.getCargo().getCommodityQuantity(Commodities.ORGANICS) <= 0 || fleet.getCargo().getCommodityQuantity(Commodities.HEAVY_MACHINERY) <= 0) {
                ...
                fleet.addFloatingText("Full of Biophotovoltaics", Misc.setAlpha(entity.getIndicatorColor(), 255), 0.5f);
                deactivate();
            } else {
                ...
                fleet.getCargo().removeCommodity(Commodities.ORGANICS, cost*ORGANICSCost*basedmodifier);
                fleet.getCargo().removeCommodity(Commodities.HEAVY_MACHINERY, cost*HeavyMachineryCost*basedmodifier);
                    ...
                    fleet.getCargo().removeCommodity(Commodities.ORGANICS, cost*ORGANICSCost);
                    fleet.getCargo().removeCommodity(Commodities.HEAVY_MACHINERY, cost*HeavyMachineryCost);
                }
                if (affectOutput) {fleet.getCargo().addCommodity(jydr_Items.BIOPHOTOVOLTAICS, cost*getBIOPHOTOVOLTAICSPerORGANICS()*basedmodifier);} else {fleet.getCargo().addCommodity(jydr_Items.BIOPHOTOVOLTAICS, cost*getBIOPHOTOVOLTAICSPerORGANICS());}
            ...
            if(fleet.getCargo().getCommodityQuantity(Commodities.ORGANICS) <= 0 || fleet.getCargo().getCommodityQuantity(Commodities.HEAVY_MACHINERY) <= 0) {
                fleet.addFloatingText("Out of Organics or Heavy Machinery", Misc.setAlpha(entity.getIndicatorColor(), 255), 0.5f);
                deactivate(); 
            } else if(supply >= fleet.getCargo().getMaxCapacity()) {
                fleet.addFloatingText("Full of Biophotovoltaics", Misc.setAlpha(entity.getIndicatorColor(), 255), 0.5f);
                deactivate();
                ...
                float basedmodifier = iCalculateBonus();
                if (affectInput) {fleet.getCargo().removeCommodity(Commodities.ORGANICS, cost*ORGANICSCost*basedmodifier);fleet.getCargo().removeCommodity(Commodities.HEAVY_MACHINERY, cost*HeavyMachineryCost*basedmodifier);} else {fleet.getCargo().removeCommodity(Commodities.ORGANICS, cost*ORGANICSCost);fleet.getCargo().removeCommodity(Commodities.HEAVY_MACHINERY, cost*HeavyMachineryCost);}
                if (affectOutput) {fleet.getCargo().addCommodity(jydr_Items.BIOPHOTOVOLTAICS, cost*getBIOPHOTOVOLTAICSPerORGANICS()*basedmodifier);} else {fleet.getCargo().addCommodity(jydr_Items.BIOPHOTOVOLTAICS, cost*getBIOPHOTOVOLTAICSPerORGANICS());}
                for (FleetMemberViewAPI view : getFleet().getViews()) {
                    view.getContrailColor().shift("timidhavenoidea", CONTRAIL_COLOR, getActivationDays(), 2, 1f);
                    view.getContrailWidthMult().shift("timidhavenoidea", 6, getActivationDays(), 2, 1f);
    ...
    public float iCalculateBonus() {
        ...
        float iMaxBonus = PristineORGANICSMultiplier*iShipSize+SalvageModifier*iSalvageCoomer;
        if (iCorrupted > iShipSize) {
            iCorrupted = iShipSize;
        };
        float iBonus = CorruptedORGANICSMultiplier*iCorrupted+PristineORGANICSMultiplier*iPristine+SalvageModifier*iSalvageCoomer;
        if (iBonus > iMaxBonus) {
            iBonus = iMaxBonus;
        };
        return iBonus+1;
